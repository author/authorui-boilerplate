...
await fs.writeFile(path.join(dist, 'index.html'), minifyHtml.minify(await fs.readFile(path.join(src, 'index.html')), {
...
const { code, map } = bundleCSS({
  ...
  minify: true,
  sourceMap: true
}), cssOutput = path.resolve(dist, 'stylesheets')


await fs.emptyDir(cssOutput)

await Promise.all([
  fs.promises.writeFile(`${cssOutput}/index.css`, code),
  fs.promises.writeFile(`${cssOutput}/index.css.map`, map)
])
